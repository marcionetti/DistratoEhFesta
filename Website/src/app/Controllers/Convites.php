...
class Convites extends BaseController
{
    ...
    public function index()
    {
        ...
        echo view('convites/index');
        echo view('fragments/footer');
    }
    public function CVD()
    {
        echo view('fragments/header');
        echo view('convites/cvd');
        echo view('fragments/footer');
    }
    public function lista()
    {
        echo view('fragments/header');
        echo view('convites/lista');
        echo view('fragments/footer');
    }
    public function listarVW()
    {
        $codUser = session()->get('Cod');
        $result = $this->cadConviteVW->getWhere(['cadUserID' => $codUser, 'TipoConviteID' => '1'])->getResult();
    ...
    public function listarCVD()
    {
        ...
        $result = $this->ConviteConvidadoVW->orderBy('conv_DataEvento DESC')->getWhere(['UserEmail' => $EmailUser, 'conv_Status >=' => '3', 'TipoConviteID' => '1'])->getResult();
        echo json_encode($result);
    }
    public function convite($cvtPar = '0')
    {
        ...
        if ($EventoCod != '00000' and $result) {
            ...
            if ($codUser != $result->cadUserID) {
                ...
                $dspBotoes .= '<a class="btn btn-app bg-purple" title="Adicionar convidado" data-toggle="modal" data-target="#modalAddConvidado"><i class="fa fa-user-plus"></i> + Conv</a>';
            }
        ...
        echo view('fragments/header');
        echo view('convites/convite', $data);
        echo view('fragments/footer');
    }
    public function updConvite()
    {
        ...
        if ($this->request->getFile('customFile')) {
    ...
    public function PromEvento()
    {
        ...
        $updConvite = $this->cadConvite->updCodConvite($ConviteCod, $dados);
    ...
    public function addConviado()
    {
        $ConviteCod = $this->request->getVar('ConviteCod');
        $Convidado = $this->request->getVar('Convidado');
        ...
        if ($Convidado) {
            $sql = "SELECT Confirmado, DataConfirmado FROM lnkConviteConvidado where ConviteCod = '$ConviteCod' and UserEmail='$Convidado'";
